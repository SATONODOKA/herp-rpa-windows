    ...
    extractCurrentCompany(educationCareerDetails) {
        ...
        for (let i = 0; i < Math.min(3, sortedCareerEntries.length); i++) {
            ...
            const companyPatterns = [
                // パターン1: 株式会社等+入社・転職（空白は任意）
                /([^\s（\n]*(?:株式会社|有限会社|合同会社|合資会社|合名会社|一般社団法人|一般財団法人|公益社団法人|公益財団法人))\s*(?:入社|転職)/,
                // パターン2: その他法人格+入社・転職（空白は任意）
                /([^\s（\n]*(?:会社|法人|グループ|ホールディングス|コーポレーション))\s*(?:入社|転職)/,
                // パターン3: 「入社」の前の単語を会社名として抽出（より広範囲）
                /([^\s（\n]+)\s*(?:入社|転職)/,
                // パターン4: 株式会社等（空白や記号の前まで）
                /([^\s（\n]*(?:株式会社|有限会社|合同会社|合資会社|合名会社|一般社団法人|一般財団法人|公益社団法人|公益財団法人))(?:\s|（|$)/,
                // パターン5: その他法人格（空白や記号の前まで）
                /([^\s（\n]*(?:会社|法人|グループ|ホールディングス|コーポレーション))(?:\s|（|$)/
            ];
            ...
            for (let j = 0; j < companyPatterns.length; j++) {
                ...
                if (match) {
                    ...
                    // 会社名の前後の余分な文字を除去
                    company = company.replace(/^[^\w\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]+/, ''); // 先頭の記号除去
                    company = company.replace(/[^\w\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]+$/, ''); // 末尾の記号除去
